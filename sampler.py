...
class ResShiftSampler(BaseSampler):
    def sample_func(self, y0, noise_repeat=False, mask=False):
        ...
        if not (ori_h % offset == 0 and ori_w % offset == 0):
            ...
            # y0 = F.pad(y0, pad=(0, pad_w, 0, pad_h), mode='reflect') # original line
            y0 = F.pad(y0, pad=(0, pad_w, 0, pad_h), mode='replicate') # changing from reflect to replicate
            print("after padding y0 shape",y0.shape)
    ...
    def inference(self, in_path, out_path, mask_path=None, mask_back=True, bs=1, noise_repeat=False):
        ...
        if in_path.is_dir():
            ...
            print("Else part where if input path is image not a directory")
